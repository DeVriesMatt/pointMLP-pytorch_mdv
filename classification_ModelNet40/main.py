...
def parse_args():
    ...
    parser.add_argument(
        "--model", default="PointNet", help="model name [default: pointnet_cls]"
    )
    parser.add_argument(
        "--epoch", default=300, type=int, help="number of epoch in training"
    )
    parser.add_argument("--num_points", type=int, default=1024, help="Point Number")
    parser.add_argument(
        "--learning_rate", default=0.1, type=float, help="learning rate in training"
    )
...
def main():
    ...
    if not os.path.isfile(os.path.join(args.checkpoint, "last_checkpoint.pth")):
        save_args(args)
        logger = Logger(
            os.path.join(args.checkpoint, "log.txt"), title="ModelNet" + args.model
        )
        logger.set_names(
            [
                "Epoch-Num",
    ...
    scheduler = CosineAnnealingLR(
        optimizer, args.epoch, eta_min=args.min_lr, last_epoch=start_epoch - 1
    )
