...
def parse_args():
    ...
    parser.add_argument(
        "--model", default="model31A", help="model name [default: pointnet_cls]"
    )
    parser.add_argument(
        "--num_classes",
        default=40,
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate the original testing result."
    )
...
def main():
    ...
    if args.validate:
        ...
        print(
            f"Note 1: Please also load the random seed parameter (if forgot, see out.txt).\n"
    ...
    for i in range(args.NUM_PEPEAT):
        ...
        for batch_idx, (data, label) in enumerate(testloader):
            ...
            for v in range(args.NUM_VOTE):
                ...
                with torch.no_grad():
                    pred += F.softmax(
                        net(new_data.permute(0, 2, 1)), dim=1
                    )  # sum 10 preds
        ...
        outstr = (
            "Voting %d, test acc: %.3f, test mean acc: %.3f,  [current best(all_acc: %.3f mean_acc: %.3f)]"
