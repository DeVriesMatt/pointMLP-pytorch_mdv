...
from cell_dataset import VesselMNIST3D
import argparse
import os
...
if __name__ == "__main__":
    ...
    name_net = output_path + "pointmlpelite_foldingTearingVersion_autoencoder_allparams"
    ...
    vessel_train = VesselMNIST3D(points_dir, centre=True, scale=20.0, partition="train")
    vessel_val = VesselMNIST3D(points_dir, centre=True, scale=20.0, partition="val")
    vessel_test = VesselMNIST3D(points_dir, centre=True, scale=20.0, partition="test")

    train_dataloader = DataLoader(vessel_train, batch_size=batch_size, shuffle=True)
    val_dataloader = DataLoader(vessel_val, batch_size=batch_size, shuffle=True)
    test_dataloader = DataLoader(vessel_test, batch_size=batch_size, shuffle=True)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            with torch.set_grad_enabled(True):
                output, embedding, _ = net(inputs.permute(0, 2, 1))
                optimizer.zero_grad()
