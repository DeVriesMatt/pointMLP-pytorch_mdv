...
def train(args, io):
    ...
    if args.use_sgd:
        print("Use SGD")
        opt = optim.SGD(
            model.parameters(), lr=args.lr * 100, momentum=args.momentum, weight_decay=0
        )
    else:
        print("Use Adam")
        opt = optim.Adam(
    ...
    for epoch in range(args.epochs):
        ...
        train_epoch(
            train_loader, model, opt, scheduler, epoch, num_part, num_classes, io
        )
        ...
        if avg_class_iou > best_class_iou:
            ...
            for cat_idx in range(16):
                io.cprint(
                    classes_str[cat_idx] + " iou: " + str(total_per_cat_iou[cat_idx])
                )
...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(
        ...
        # target: b,n
        seg_pred = model(
            points, norm_plt, to_categorical(label, num_classes)
        )  # seg_pred: b,n,50
        loss = F.nll_loss(
            seg_pred.contiguous().view(-1, num_part), target.view(-1, 1)[:, 0]
        )

        # instance iou without considering the class average at each batch_size:
        batch_shapeious = compute_overall_iou(
            seg_pred, target, num_part
        ...
        batch_shapeious = seg_pred.new_tensor(
            [np.sum(batch_shapeious)], dtype=torch.float64
        ...
        ).sum()  # torch.int64: total number of correct-predict pts
        ...
        accuracy.append(
            correct.item() / (batch_size * num_point)
...
def test_epoch(test_loader, model, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(
        enumerate(test_loader), total=len(test_loader), smoothing=0.9
    ):
        batch_size, num_point, _ = points.size()
        points, label, target, norm_plt = (
            Variable(points.float()),
        ...
        for shape_idx in range(seg_pred.size(0)):  # sample_idx
            ...
            final_total_per_cat_iou[cur_gt_label] += batch_shapeious[
                shape_idx
            ]  # add the iou belongs to this cat
            final_total_per_cat_seen[
                cur_gt_label
            ] += 1  # count the number of this cat is chosen
    ...
    for cat_idx in range(16):
        if (
            final_total_per_cat_seen[cat_idx] > 0
...
def test(args, io):
    ...
    state_dict = torch.load(
        "checkpoints/%s/best_%s_model.pth" % (args.exp_name, args.model_type),
        map_location=torch.device("cpu"),
    )["model"]
    ...
    for cat_idx in range(16):
        if total_per_cat_seen[cat_idx] > 0:
            total_per_cat_iou[cat_idx] = (
                total_per_cat_iou[cat_idx] / total_per_cat_seen[cat_idx]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lr", type=float, default=0.003, metavar="LR", help="learning rate"
    )
    parser.add_argument(
        "--momentum",
        type=float,
    ...
    parser.add_argument(
        "--manual_seed", type=int, metavar="S", help="random seed (default: 1)"
    )
    ...
    if not args.eval:
        io = IOStream(
            "checkpoints/" + args.exp_name + "/%s_train.log" % (args.exp_name)
        )
    else:
